...
const buttonVariants = cva(
  'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 animated-border relative',
  {
    variants: {
      variant: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, ...props }, ref) => {
    // Don't apply animated-border for link variant
    const baseClasses = buttonVariants({ variant, size });
    const classes = variant === 'link' 
      ? baseClasses.replace(/animated-border relative/g, '').replace(/border border-[^ ]+/g, '')
      : baseClasses;
